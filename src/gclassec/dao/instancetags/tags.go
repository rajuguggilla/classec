...
func InstanceProvider(w http.ResponseWriter, r *http.Request) {
	...
	for _,element := range  dbresponse{
		for _,r := range inst{
			...
			if r.InstanceName != "" {
				user = tagstruct.Providers{InstanceId:instanceId, InstanceName:r.InstanceName, Cloud:r.Cloud, Tagname:r.Tagname}

			}else {
				user = tagstruct.Providers{InstanceId:instanceId, Cloud:r.Cloud, Tagname:r.Tagname}
