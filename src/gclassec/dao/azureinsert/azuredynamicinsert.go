	...
	for _, element := range *ls.Value {
		...
		for _, element1 := range *dlist.Value {
			if element1.Data[len(element1.Data) - 1].Average == nil{
				azure_dynamic := azurestruct.AzureDynamic{Name:*element.Name, VmId:*element.VMID, Timestamp:*(element1.Data[len(element1.Data) - 1].TimeStamp), Minimum:0.0, Maximum:0.0, Average:0.0, Unit:*element1.Unit}
				db.Create(&azure_dynamic)
			}else{
				azure_dynamic := azurestruct.AzureDynamic{Name:*element.Name, VmId:*element.VMID, Timestamp:*(element1.Data[len(element1.Data) - 1].TimeStamp), Minimum:*(element1.Data[len(element1.Data) - 1].Minimum), Maximum:*(element1.Data[len(element1.Data) - 1].Maximum), Average:*(element1.Data[len(element1.Data) - 1].Average), Unit:*element1.Unit}
				db.Create(&azure_dynamic)
			}
		}
		}
